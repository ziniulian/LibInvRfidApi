...
public class WriteUserData_6C extends BaseMessage implements IEventHandle {
	...
	public WriteUserData_6C(byte antenna, byte[] pwd, int ptr, byte[] userData) {
		...
		if (udLen > maxWriteLen * 2) {
			...
			System.arraycopy(ptrArray, 0, super.msgBody, 1 + pwd.length,
					ptrArray.length);
			super.msgBody[1 + pwd.length + ptrArray.length] = (byte) ((userData.length - p) / 2);

			System.arraycopy(userData, p, super.msgBody, 2 + pwd.length
					+ ptrArray.length, userData.length - p);
			...
			System.arraycopy(userData, 0, super.msgBody, 2 + pwd.length
					+ ptrArray.length, userData.length);
		}
	}
	...
	void writeUserData_6C_OnExecuting(Object sender, EventArgs e) {
		...
		for (int i = 0; i < c; i++) {
			...
			System.arraycopy(userData, i * maxWriteLen * 2, wd, 0,
					maxWriteLen * 2);

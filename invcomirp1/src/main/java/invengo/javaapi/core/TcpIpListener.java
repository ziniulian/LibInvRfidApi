      ...
      while (!(this.isStop)) {
        ...
        if (this.onClientConnHandle != null)
          for (int i = 0; i < this.onClientConnHandle.size(); ++i)
            ((IClientConnHandle)this.onClientConnHandle.get(i)).clientConnHandle(socket, this.pVer);
      }
    }
    catch (IOException e)
    {
      Util.logAndTriggerApiErr("Port:" + this.port, "", e.getMessage(), Util.LogType.Error);
    } finally {
      this.isStop = true;
      try {
        this.server.close();

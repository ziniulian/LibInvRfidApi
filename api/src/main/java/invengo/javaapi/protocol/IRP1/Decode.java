...
public class Decode implements IProcess {
	...
	public IMessageNotification parseMessageNoticefaction(byte[] recvMsg) {
		...
		if (MessageType.msgType.containsKey(id)) {
			try {
				msg = (IMessageNotification) Class.forName(
						"invengo.javaapi.protocol.IRP1."
								+ MessageType.msgType.get(id)).newInstance();
			...
			while (i < m_recvBytes.length) {
				...
				if (m_recvBytes[i] <= 5)// 查找到第一个00H
				{
					...
					if (i <= m_recvBytes.length - 5)// 算长度
					{
						...
						if (m_recvBytes[i] > 0) {
							orderLength = m_recvBytes[i] << 8
									| m_recvBytes[i + 1] & 0xFF;
						}
					} else {
						this.recvbuff = new byte[m_recvBytes.length - i];
						System.arraycopy(m_recvBytes, i, recvbuff, 0,
								recvbuff.length);
						if (i != lostDataStartIndex) {
							byte[] lostData = new byte[i - lostDataStartIndex];
							System.arraycopy(m_recvBytes, lostDataStartIndex,
									lostData, 0, lostData.length);
							Log
									.debug("丢失的数据1:"
											+ Util
											.convertByteArrayToHexString(lostData));
						}
						break;
					}
					// 针对指令长度做相应的处理
					...
					if (orderLength + 4 <= m_recvBytes.length - i) {
						...
						System.arraycopy(m_recvBytes, i, m_buff, 0,
								m_buff.length);
						if (CRCClass.validateCRC16(m_buff)) {
							...
							for (int k = i + 1; k < 4 + orderLength + i; k++) {
								if (m_recvBytes[k] <= 0x05) {
									if (k + 1 < 4 + orderLength + i
											&& m_recvBytes[k] == 0
											&& m_recvBytes[k + 1] == 0) {
										continue;
									}
									is00 = true;
									byte[] lostData = new byte[k
											- lostDataStartIndex];
									System.arraycopy(m_recvBytes,
											lostDataStartIndex, lostData, 0,
											lostData.length);
									Log
											.debug("丢失的数据2:"
													+ Util
							...
							if (!is00) {
								byte[] lostData = new byte[m_recvBytes.length
										- lostDataStartIndex];
								System.arraycopy(m_recvBytes,
										lostDataStartIndex, lostData, 0,
										lostData.length);
							...
							for (int ii = 1; ii < this.recvbuff.length; ii++) {
								if (this.recvbuff[ii] <= 0x05) {
									...
									if (ii <= recvbuff.length - 5)// 算长度
									{
										...
										if (recvbuff[ii] > 0) {
											iiorderLength = recvbuff[ii] << 8
													| recvbuff[ii + 1] & 0xFF;
										}
									}

									if (iiorderLength != 0
											&& iiorderLength + 4 <= recvbuff.length
											- ii) {
										byte[] iim_buff = new byte[4 + iiorderLength];
										System.arraycopy(recvbuff, ii,
												iim_buff, 0, iim_buff.length);
				...
				if (i == m_recvBytes.length - 1) {
					byte[] lostData = new byte[m_recvBytes.length
							- lostDataStartIndex];
					System.arraycopy(m_recvBytes, lostDataStartIndex, lostData,
							0, lostData.length);
					Log.debug("丢失的数据5:"
							+ Util.convertByteArrayToHexString(lostData));
				}
				i++;
			}
		}

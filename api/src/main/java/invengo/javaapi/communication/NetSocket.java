...
public class NetSocket extends ICommunication {
	...
	private void open() {
		...
		try {
			...
			Util.logAndTriggerApiErr(super.readerName, "FF19",
					e.getMessage(),
					LogType.Fatal);
		...
		if (socket == null) {
			if (connString.indexOf(':') != -1) {
				this.hostName = connString
						.substring(0, connString.indexOf(':'));
				this.portNum = Integer.parseInt(connString.substring(connString
						.indexOf(':') + 1));
			} else {
				this.hostName = connString;
			}
	...
	public int send(byte[] data) {
		if (!super.isConnected
				|| (socket == null || (socket != null && !socket.isConnected()))) {
			super.isConnected = false;
			Util.logAndTriggerApiErr(super.readerName, "FF22", "",
					LogType.Warn);
			return 0;
		}
		int sl = 0;
		if (data != null) {
			try {
				...
				Util.logAndTriggerApiErr(super.readerName, "FF21", e
						.getMessage(), LogType.Error);
				sl = 0;
			}
		} else {
			Util.logAndTriggerApiErr(super.readerName, "FF23", "",
					LogType.Info);
		}
		return sl;
	}
	...
	private void runClient() {
		try {
			...
			Log.debug(super.readerName + " is running.Connected:"
					+ String.valueOf(super.isConnected));
			while (super.isConnected()) {
				...
				if (bytesRead > 0) {
					...
					Log.debug("RXD:"
							+ Util.convertByteArrayToHexString(receBytes));
					super.setBufferQueue(receBytes);
				} else {
					super.setConnected(false);
					Util.logAndTriggerApiErr(super.readerName, "FF22",
							ErrorInfo.errMap.get("FF24"),
							LogType.Warn);
					break;
				}
			}

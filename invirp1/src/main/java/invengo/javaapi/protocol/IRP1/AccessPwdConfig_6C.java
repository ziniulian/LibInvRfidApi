...
public class AccessPwdConfig_6C extends BaseMessage implements IEventHandle {
	...
	public AccessPwdConfig_6C(byte antenna, byte[] oldPwd, byte[] newPwd) {
		...
		super.msgBody = new byte[1 + oldPwd.length + newPwd.length];
		super.msgBody[0] = antenna;
		System.arraycopy(oldPwd, 0, msgBody, 1, oldPwd.length);
		System.arraycopy(newPwd, 0, msgBody, 1 + oldPwd.length, newPwd.length);
	}

...
public class ReadReserved_6C extends BaseMessage implements IEventHandle {
	 ...
	 *            标签数据读取长度
	 */
	public ReadReserved_6C(byte antenna, byte[] pwd, int ptr, byte length) {
		int myPtr = ptr;
		...
		// 1-4字节为保留域
		System.arraycopy(pwd, 0, super.msgBody, 1, pwd.length <= 4 ? pwd.length : 4);
		System.arraycopy(ptrArray, 0, msgBody, 5, ptrArray.length);
		super.msgBody[5 + ptrArray.length] = myLength;
	}

	public ReadReserved_6C(byte antenna, byte[] pwd, int ptr, byte length, byte[] tagID,
						   MemoryBank tagIDType) {
		this(antenna, pwd, ptr, length);
		//		eventFlag = 2;
		this.tagID = tagID;
	...
	void readUserData_6C_OnExecuting(Object sender, EventArgs e) {
		...
		for (int i = 0; i < c; i++) {
			...
			ReadReserved_6C r = new ReadReserved_6C(antenna, this.pwd, p, (byte) maxReadLen);
			if (reader.send(r)) {

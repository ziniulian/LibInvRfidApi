	...
	public class ReceivedInfo extends invengo.javaapi.core.ReceivedInfo {
		...
		public ReceivedInfo(byte[] buff, Reader reader, IMessageNotification msg) {
			...
			if (msgType.equals("RXD_ID_6B")) {
				...
				//				System.out.println("Before - " + Util.convertByteArrayToHexString(myEpc) + " - Rssi - " + reader.isRssiEnable);
				if (reader.readerType.equals("500")) {
					...
					this.epc = myEpc;
					//					System.out.println("After - " + Util.convertByteArrayToHexString(myEpc));
				}
				...
				if (reader.readerType.equals("500") && mytid.length > 2) {
					...
					if (reader.isUtcEnable && mytid.length > 8) {
						this.rxdtime = new byte[8];
						byte[] t_tid = new byte[mytid.length - 8];
						System.arraycopy(mytid, 0, t_tid, 0, t_tid.length);
						System.arraycopy(mytid, t_tid.length, this.rxdtime, 0, 8);
						mytid = t_tid;
					}
					this.tid = mytid;
				...
				this.userdata = m.getReceivedMessage().getUserData();
				this.antenna = m.getReceivedMessage().getAntenna();
				this.temperature = m.getReceivedMessage().getTemperature();
				...
				if (reader.readerType.equals("800")) {
					if (reader.isUtcEnable && myepc.length > 8) {
						...
						System.arraycopy(myepc, 0, t_epc, 0, t_epc.length);
						this.rxdtime = new byte[8];
						System.arraycopy(myepc, t_epc.length, this.rxdtime, 0, 8);
						myepc = t_epc;
					}
					if (reader.isRssiEnable && myepc.length > 2) {
						byte[] t_epc = new byte[myepc.length - 2];
			...
			}else if(msgType.equals("RXD_EPC_TID_TEMPERATURE")){//配合铁路产品部门新增C1指令解析
				RXD_EPC_TID_TEMPERATURE m = (RXD_EPC_TID_TEMPERATURE) msg;
				this.tagType = "6C";
				this.tid = m.getReceivedMessage().getTID();
				this.antenna = m.getReceivedMessage().getAntenna();
				this.temperature = Double.valueOf(m.getReceivedMessage().getTemperature());

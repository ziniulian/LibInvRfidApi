...
public class GBWriteTag extends BaseMessage implements IEventHandle {
	...
	/**该指令用于写指定标签指定区域指定首地址指定长度的数据,其中指定长度为数据长度,单位为字<br>
	 * <p>
	 * 1.写指令.data.length>0x20时需要分段写入,必须先设置匹配数据和匹配区域参数,需要指定写入的数据data.<br>
	 * <p>
	 * 用法:<br>
	 * 写指令:1)length>0x20时<br>
	 GBWriteTag message = new GBWriteTag(antenna, password, bank, headAddress, length, data);<br>
	 ...
	 * @param data	写入数据
	 */
	public GBWriteTag(int antenna, String password, GBMemoryBank bank, int headAddress, byte[] data) {
	...
	private void writeTag_OnExecuting(Object sender, EventArgs e) {
		...
		for(int i = 0; i < loop; i++){
			...
			GBWriteTag currentMessage = new GBWriteTag(
